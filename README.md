...
- **LOBSTER multicenter COBI (orbitalwise)** — browse every multicenter (Nc) `cobiBetween` chain in a `COBICAR.lobster` file, independent of any phonon data, with per-orbital-combination breakdown (grouped by s/p/d/f or full m-resolved), a "hide contributions with nothing going on" threshold, shared-plot overlay, and SVG/PDF/PNG plot or CSV export; available as the **LOBSTER COBI** tab in the GUI — see [docs/lobster-cobi.md](docs/lobster-cobi.md)
